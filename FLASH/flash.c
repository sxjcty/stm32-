...
u16 SPI_FLASH_TYPE=W25Q16;//默认就是25Q16
...
{	
  ...
  GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);//IO口重映射：屏蔽JTAG引脚，只留SWD引脚
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	...
 	GPIO_SetBits(GPIOA,GPIO_Pin_15);//IO设置为高
	...
	while(SPI_Flash_ReadID()!=W25Q16)								//????W25Q16
		{
			printf("未检测到W25Q16");
		}
...
void SPI_Flash_Write_Byte(u8 data,u32 WriteAddr)
{
    u8 Data[10];
	  Data[0]=data;
		SPI_Flash_Write((u8*)Data,WriteAddr,1);
}



/**********************************
函数功能：读取Flash中的某一位写数据
函数参数：
					WriteAddr  要存入数据的地址
返回值 ： 要读取的数据
********************************/
u8 SPI_Flash_Read_Byte(u32 WriteAddr)
{
    u8 Data[10];
		SPI_Flash_Read((u8*)Data,WriteAddr,1);
		//printf("Data[0]=%d\r\n",Data[0]);
	  return Data[0];
}
